...
- [السكربتات المساعدة](#-السكربتات-المساعدة)
...
هذا المستودع يحتوي على مجموعة أدوات متكاملة لتحويل سيرفر Linux خام إلى بيئة مُحصّنة بالكامل وجاهزة للإنتاج. يشمل:

1. **Ansible Playbook** لإعداد السيرفر بالكامل تلقائياً
2. **سكربت فحص شامل** يتحقق من 30+ خدمة ومكون أمني
3. **سكربت فحص Webhook** للتحقق من عمل نظام التنبيهات
4. **سكربت إصلاح تلقائي** لمعالجة المشاكل المكتشفة

يتعامل الـ Playbook مع ترتيب العمليات بذكاء — فيُثبّت Docker ويُحدّث قواعد بيانات ClamAV **قبل** تفعيل جدار الحماية UFW، ويضمن حماية الاتصالات النشطة عبر قواعد `ESTABLISHED/RELATED` لمنع انقطاع جلسة SSH أثناء التنفيذ.
...
```
setup_servers/
├── ultimate-secure-setup.yml   # 🎯 Ansible Playbook الرئيسي (1342 سطر)
├── check-server.sh             # 🔍 سكربت فحص شامل للسيرفر (1309 سطر)
├── check_webhook.sh            # 🔗 سكربت فحص Webhook (470 سطر)
├── fix_ultimate_setup.sh       # 🔧 سكربت إصلاح تلقائي (1128 سطر)
└── README.md                   # 📖 التوثيق
```

| الملف | الوصف | متى تستخدمه |
|-------|-------|-------------|
| `ultimate-secure-setup.yml` | الـ Playbook الرئيسي — يُحصّن السيرفر بالكامل | عند إعداد سيرفر جديد |
| `check-server.sh` | فحص 30 قسم: من الحزم والخدمات إلى Sysctl و AppArmor | للتحقق من صحة السيرفر بعد الإعداد أو دورياً |
| `check_webhook.sh` | فحص DNS + SSL + HTTP GET/POST + Headers + Alertmanager config | للتحقق من عمل نظام التنبيهات |
| `fix_ultimate_setup.sh` | إصلاح 12 مشكلة شائعة بناءً على نتائج الفحص | عند وجود مشاكل بعد التشغيل |
...
| **فحص شامل** | سكربت يفحص 30+ مكون أمني مع تقييم (A+ إلى D) |
| **إصلاح تلقائي** | سكربت يعالج 12 مشكلة شائعة تلقائياً |
...
### 🔍 سكربت الفحص الشامل (`check-server.sh`)

سكربت شامل يفحص **30 قسم** من مكونات السيرفر ويعطي تقييم نهائي:
...
| # | القسم | الوصف |
|---|-------|-------|
| 1 | الحزم الأساسية | 40+ حزمة مطلوبة |
| 2 | SSH | إعدادات الأمان (Root Login, Key Auth, MaxAuthTries) |
| 3 | UFW | حالة الجدار وقواعده |
| 4 | Fail2Ban | Jails النشطة وإحصائيات الحظر |
| 5 | ClamAV | Daemon + Freshclam + قاعدة البيانات |
| 6 | Rootkit Hunter | rkhunter + chkrootkit |
| 7 | Lynis | تدقيق أمني + Hardening Index |
| 8 | AIDE | كشف تغيير الملفات |
| 9 | أدوات المراقبة | Prometheus, Grafana, Alertmanager, Netdata, Monit |
| 10 | سكربت المراقبة | المخصص + Systemd Timers |
| 11 | Docker | الحاويات + Compose |
| 12 | خوادم الويب | Nginx + Apache |
| 13 | SSL/TLS | شهادات Let's Encrypt + التجديد التلقائي |
| 14 | قواعد البيانات | MySQL, PostgreSQL, Redis, MongoDB |
| 15 | النسخ الاحتياطي | أدوات + جدولة |
| 16 | إدارة السجلات | Logrotate + Rsyslog + Journal |
| 17 | Auditd | تدقيق النظام |
| 18 | Sysctl | 12 إعداد أمني للـ Kernel |
| 19 | AppArmor/SELinux | حماية التطبيقات |
| 20 | الشبكة | واجهات + DNS + اتصال الإنترنت |
| 21 | المستخدمون | صلاحيات + UID 0 + ملفات حساسة |
| 22 | الموارد | القرص + الذاكرة + Load Average |
| 23 | NTP | مزامنة الوقت |
| 24 | التحديثات التلقائية | Unattended Upgrades |
| 25 | WAF | ModSecurity |
| 26 | VPN | WireGuard + OpenVPN |
| 27 | Cron Jobs | المهام المجدولة |
| 28 | أدوات أمنية | Tripwire, Tiger, Debsums |
| 29 | خدمات فاشلة | systemd failed services |
| 30 | المنافذ | المنافذ المفتوحة والمستمع عليها |

**نظام التقييم:**
- 🏆 **A+ (90%+)** — حماية ممتازة
- ⭐ **A (80%+)** — حماية جيدة جداً
- 👍 **B (70%+)** — حماية جيدة
- ⚡ **C (60%+)** — يحتاج تحسين
- ⚠️ **D (<60%)** — يحتاج إصلاح عاجل

### 🔗 سكربت فحص Webhook (`check_webhook.sh`)

يتحقق من عمل نظام التنبيهات بالكامل:
...
**يفحص:**
1. **DNS Resolution** — حل اسم النطاق
2. **الاتصال** — ping + فحص البورتات 80/443
3. **شهادة SSL** — صلاحية + تاريخ الانتهاء
4. **HTTP GET** — استجابة الـ webhook
5. **HTTP POST** — إرسال تنبيه اختباري
6. **Response Headers** — فحص الـ headers
7. **الخدمات المحلية** — Nginx + Alertmanager
8. **إعدادات Alertmanager** — التحقق من الـ config

> **ملاحظة:** عدّل المتغير `WEBHOOK_URL` في بداية السكربت ليطابق رابط الـ webhook الخاص بك.

### 🔧 سكربت الإصلاح التلقائي (`fix_ultimate_setup.sh`)

يعالج **12 مشكلة شائعة** تلقائياً:
...
| # | المشكلة | الإصلاح |
|---|---------|---------|
| 1 | حزم مفقودة | تثبيت 20+ حزمة |
| 2 | ClamAV فاشل | إصلاح freshclam + daemon + الصلاحيات |
| 3 | CrowdSec متوقف | إعادة تثبيت + إعداد Collections |
| 4 | AIDE Daily Check | إنشاء قاعدة البيانات + إصلاح الخدمة |
| 5 | UFW غير مفعل | تفعيل مع قواعد SSH/HTTP/HTTPS |
| 6 | X11Forwarding مفعل | تعطيله في SSH config |
| 7 | Sysctl خاطئ | إصلاح 3 إعدادات أمنية |
| 8 | NTP غير مفعل | تفعيل مزامنة الوقت |
| 9 | Nginx | إعداد Headers أمنية |
| 10 | Monit + Lynis | إعداد المراقبة + التدقيق الأسبوعي |
| 11 | Certbot | إعداد التجديد التلقائي |
| 12 | Fail2Ban + المراقبة | تحسين القواعد + سكربت مراقبة متقدم |
...
# التقرير التلقائي (كل 5 دقائق بعد تشغيل fix_ultimate_setup.sh)
...
### سكربت الفحص يُظهر تقييم منخفض

```bash
# شغّل سكربت الإصلاح التلقائي
sudo bash fix_ultimate_setup.sh
...
- عدّل `WEBHOOK_URL` في `check_webhook.sh` ليطابق رابطك قبل استخدامه
